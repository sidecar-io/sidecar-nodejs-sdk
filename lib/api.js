...
var keyId = 'K8AWQMJ28/H6SQGDZJIS';
var secret = '2oyAYTm08K+oMIHzdFggqqHQR96worjRJXzHQeGo';

// User access keys
var userKeyId = 'NLTWSYZYBUAL+DWS3OUA';
var userSecret = '/Wq++KXcpAFGi6LHXOW0Z16G/Z9x1ZLC6MoI+BQL';

// User Device ID
var deviceId = '8f80e970-6b57-11e4-9803-0800200c9a66';
...
module.exports = {
   ...
   *
   */
  provisionUser: function(payload, callback){
    ...
    var signature = crypto.createHmac('sha1', secret).update(toSign).digest('base64')
    request({
      method: httpMethod,
      url: hostURL + uriPath,
      headers: {
        ...
        'Signature-Version': '1',
        'Content-MD5': md5,
        'Authorization': 'SIDECAR '+ keyId + ': ' + signature
    },
    body: payload
    }, function(error, response, body){
      if(response.statusCode == 200 && body != '[]' && body){
        var userKeys = JSON.parse(body);
        callback(error, userKeys);
      }
      else
      {
        console.log('provisionUser body: ' + body);
        console.log('provisionUser response: ' + response.statusCode); 
      }
    });
  },
   ...
   *
   */
  getUserKeys: function(payload, callback){
    payload = JSON.stringify(payload);
    var httpMethod = 'POST';
    ...
    body: payload
      ...
      {
        console.log('getUserKeys body: ' + body);
        console.log('getUserKeys response: ' + response.statusCode); 
      }
    });
  },
   ...
   * 
   */
  publishEvent: function(payload, currentTime, callback){
   ...
   * 
   */
  queryLatestEvent: function(payload, currentTime, callback){
    var httpMethod = 'POST';
   ...
   * 
   */
  queryStats: function(payload, currentTime, callback){
    var httpMethod = 'POST';
